...
#testing_percentage和validation_percentage,参数指定了测试数据集和验证数据集的大小
def create_image_lists(testing_percentage,validation_percentage):
    #得到的所有图片都存在result这个字典dictionary里。这个字典的key为类别的名称。
...
#image_dir参数给出了根目录。存放图片数据的根目录和存放图片特征向量的个目录地址不同
#label_name参数给定了类别的名称
#index参数给定了需要获取的图片的编号
#category参数制定了需要获取的图片是在训练数据集、测试数据集还是验证数据集
...
def run_bottleneck_on_image(sess,image_data,image_data_tensor,bottleneck_tensor):
    #这个过程实际上就是讲当前图片作为输入计算瓶颈张量的值。这个瓶颈张量的值就是
    #这张图片的新的特征向量
...
def get_or_create_bottleneck(sess,image_lists,label_name,index,category,
    ...
    #如果这个特征向量文件不存在，则通过Inception-v3模型来计算特征向量，并
    #将计算的结果存入文件
    if not os.path.exists(bottleneck_path):
        ...
        #将计算得到的特征向量存入文件
        bottlencek_string=','.join(str(x) for x in bottleneck_values)
        with open(bottleneck_path,'w') as bottleneck_file:
            bottleneck_file.write(bottlencek_string)
    else:
        #直接从文件中获取图片相应的特征向量
        with open(bottleneck_path,'r') as bottleneck_file:
            bottlencek_string=bottleneck_file.read()
        bottleneck_values=[float(x) for x in bottlencek_string.split(',')]
        #返回得到的特征向量
    return bottleneck_values
...
def main(_):
    ...
    #里边保存了每一个节点取值的计算方法以及变量的取值，TensorFlow模型持久化的问题在第5章中有详细的介绍
    with gfile.FastGFile(os.path.join(MODEL_DIR,MODEL_FILE),'rb') as f:
        graph_def=tf.GraphDef()
        graph_def.ParseFromString(f.read())
    #加载读取的Inception-v3模型，并返回数据输入所对应的张量以及计算瓶颈层结果所对应的张量
    bottleneck_tensor,jpeg_data_tensor=tf.import_graph_def(graph_def,return_elements=[BOTTLENECK_TENSOR_NAME,JPEG_DATA_TENSOR_NAME])
    #定义新的神经网络输入，这个输入就是新的图片经过Inception-v3模型前向传播到达瓶颈层的节点取值，
    #可以将这个过程类似地理解为一种特征提取
    bottleneck_input=tf.placeholder(tf.float32, [None,BOOTLENECK_TENSOR_SIZE], name='BottleneckInputPlaceholder')
    #定义新的标准答案输入
    ground_truth_input=tf.placeholder(tf.float32,[None,n_classes],name='GroundTruthInput')
    #定义一层全连接层来解决新的图片分类问题，因为训练好的Inception-v3模型已经将原始的图片抽象为了更加容易分类的特征向量了
    #所以不需要再训练那么复杂的神经网络来完成这个新的分类任务
    with tf.name_scope('final_training_ops'):
        weights=tf.Variable(tf.truncated_normal([BOOTLENECK_TENSOR_SIZE,n_classes],stddev=0.001))
